...
func renderLegend(profile config.NetworkProfile) string {
	...
	return strings.Join([]string{
		...
		"",
		"                   [ADR-088] In demo mode the microservice assigns every",
		"                   provider its OWN synthetic ASN, so each ASN holds at",
		"                   most one shard per segment by construction and the cap",
		"                   above never has to reject a placement. The cap is still",
		"                   enforced in code (it is the same check production uses)",
		"                   — this panel is simply not evidence that it works. In",
		"                   production ASNs come from real BGP data, many providers",
		"                   share one, and the cap is genuinely load-bearing.",
		"",
